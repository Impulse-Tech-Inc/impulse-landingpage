<template>
  <div class="relative w-full flex items-center justify-center">
    <div class="w-full max-w-md bg-slate-900/80 border border-white/10 rounded-[3rem] shadow-2xl overflow-hidden backdrop-blur-sm">
      <!-- Main Visual Area -->
      <div class="relative flex items-center justify-center py-10 px-6">
        ...
        <div class="relative z-10 w-48 h-48 flex items-center justify-center">
          ...
          <svg class="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="46" fill="none" stroke="rgba(255,255,255,0.05)" stroke-width="0.5" />
          ...
          <!-- Channel Nodes -->
          <div v-for="(node, i) in channelNodes" :key="i" class="absolute w-9 h-9 rounded-lg bg-white/10 border border-white/10 flex items-center justify-center shadow-lg" :class="node.color" :style="getNodePos(i, 3, 100)">
      ...
      <!-- Resolution Log -->
      <div class="px-5 pb-5">
        <div class="p-3.5 bg-black/40 border border-white/10 rounded-xl font-mono text-[9px] text-teal-400 space-y-1 overflow-hidden">
          <div class="flex items-center justify-between mb-1.5 border-b border-white/10 pb-1.5">
            <span class="text-white/40 uppercase tracking-widest">Resolution_Log</span>
