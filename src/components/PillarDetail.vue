<template>
  <section class="min-h-screen bg-white pt-24 pb-32 px-6 md:px-8 overflow-hidden">
    <div class="max-w-[1400px] mx-auto">
      <!-- Back Button -->
      <button
        class="group flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 hover:text-[#7F39E9] transition-all mb-8"
        @click="$emit('back')"
      >
        <div class="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center transition-all group-hover:bg-[#7F39E9] group-hover:text-white group-hover:border-[#7F39E9]">
      ...
      <div v-if="pillarId === 'network'" class="space-y-24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div class="space-y-8">
            ...
            <h1 class="text-4xl sm:text-6xl md:text-8xl font-black text-slate-900 leading-tight md:leading-[0.85] tracking-tighter">
              Telemetrics<br />Intelligence.
            </h1>
            <h2 class="text-2xl md:text-3xl font-bold text-slate-700 tracking-tight leading-tight max-w-md">{{ $t('pillarNetworkHeroSubtitle') }}</h2>
            <p class="text-xl text-slate-500 font-light max-w-xl leading-relaxed">{{ $t('pillarNetworkHero') }}</p>
            <PillarBadges />
          </div>
          <!-- Orbital Visual -->
        ...
        <div v-reveal class="p-6 md:p-10 lg:p-16 bg-white border border-slate-200 rounded-[3rem] space-y-16 shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
          ...
          <div class="flex flex-col md:flex-row items-start md:items-center gap-4">
            ...
            <div class="flex items-center gap-3 flex-wrap">
              <span class="text-sm font-black text-slate-900 uppercase tracking-wider">TELEMETRICS</span>
              <span class="text-sm text-slate-400">&middot;</span>
              <span class="text-sm text-slate-500">{{ $t('pillarNetworkModuleLabel') }}</span>
          ...
          <!-- Quote -->
          <p class="text-xl md:text-2xl text-slate-500 italic font-light leading-relaxed max-w-4xl border-l-4 border-[#7F39E9]/30 pl-6">
            {{ $t('pillarNetworkQuote') }}
          </p>

          <!-- Noise vs Signal -->
          <div class="p-4 md:p-10 lg:p-16 bg-slate-50 border border-slate-200 rounded-[2rem] relative overflow-hidden">
            <div class="relative z-10 flex flex-col md:flex-row items-center gap-12 text-center md:text-left justify-center">
              <div class="space-y-2">
                ...
                <div class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter">561</div>
                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('pillarNetworkAlarms') }}</div>
              </div>
              ...
              <div class="space-y-2">
                ...
                <div class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter">1</div>
                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('pillarNetworkRoot') }}</div>
              </div>
            </div>
          </div>
          ...
          <!-- Strategic Value -->
          <div class="space-y-6">
            <div class="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">{{ $t('pillarStrategicValue') }}</div>
          ...
          <!-- Process Flow -->
          <div class="relative flex flex-col md:flex-row items-center justify-between gap-6 md:gap-0 p-8 md:p-10 bg-slate-50 rounded-[2.5rem] border border-slate-100 overflow-hidden">
            <template v-for="(step, i) in networkProcessSteps" :key="i">
              <!-- Step Node -->
              <div class="relative z-10 flex flex-col items-center gap-4">
                <div class="w-16 h-16 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-[#7F39E9] shadow-xl hover:border-[#7F39E9] transition-all">
                  <Icon :icon="step.icon" class="text-2xl" />
                </div>
                <div class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-900">{{ step.label }}</div>
              </div>
              <!-- Connecting Line with Pulse -->
              <div v-if="i < networkProcessSteps.length - 1" class="hidden md:block flex-1 h-px bg-slate-200 relative mx-2">
              ...
              <!-- Mobile Arrow -->
              <div v-if="i < networkProcessSteps.length - 1" class="md:hidden text-slate-300">
          ...
          <!-- Impact Card -->
          <div class="p-6 md:p-12 rounded-[2rem] relative overflow-hidden" style="background: linear-gradient(224.95deg, #a446f4 -1.95%, #4138f3 104.5%); border: 1px solid rgba(255,255,255,0.15)">
        ...
        <div v-reveal class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div class="space-y-10">
            <div class="space-y-6">
              ...
              <h2 class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter leading-none">{{ $t('pillarNetworkEcoTitle1') }}<br />{{ $t('pillarNetworkEcoTitle2') }}</h2>
              <p class="text-xl text-slate-500 font-light leading-relaxed max-w-xl">{{ $t('pillarNetworkEcoDesc') }}</p>
            </div>
            <div class="flex flex-wrap gap-3">
              <span
                v-for="vendor in networkVendors"
                :key="vendor"
                class="px-5 py-2.5 rounded-2xl border border-slate-200 bg-slate-50 text-sm font-bold text-slate-700"
              >{{ vendor }}</span>
          ...
          <!-- Integration Visual -->
          <div class="relative aspect-video bg-white rounded-[3rem] border border-slate-200 overflow-hidden shadow-2xl">
            <div class="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_center,rgba(127,57,233,0.3),transparent_70%)]" />
            <div class="absolute inset-0 flex items-center justify-center p-12">
              <div class="relative w-full h-full border border-slate-100 rounded-2xl flex items-center justify-center">
                <div class="grid grid-cols-3 gap-10">
                  <div v-for="n in 6" :key="n" class="w-16 h-16 rounded-xl bg-slate-50 border border-slate-200 eco-node-pulse" :style="{ animationDelay: `${n * 0.5}s` }" />
                </div>
            ...
            <div class="absolute bottom-5 left-6 right-6 flex justify-between items-center text-[10px] font-black uppercase tracking-[0.3em] text-slate-300">
      ...
      <div v-if="pillarId === 'crm'" class="space-y-24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div class="space-y-8">
            ...
            <h1 class="text-4xl sm:text-6xl md:text-8xl font-black text-slate-900 leading-tight md:leading-[0.85] tracking-tighter">Agentic<br />CRM.</h1>
            <h2 class="text-2xl md:text-3xl font-bold text-slate-700 tracking-tight leading-tight max-w-md">{{ $t('pillarCrmHeroSubtitle') }}</h2>
            <p class="text-xl text-slate-500 font-light max-w-md leading-relaxed">{{ $t('pillarCrmHeroDesc') }}</p>
            <PillarBadges />
          </div>
          <div class="rounded-[2.5rem] overflow-hidden border border-slate-200 shadow-2xl">
            <StoryLaneEmbed />
          </div>
        </div>

        <!-- SECTION 2: ENGAGE - NETWORK-AWARE CRM -->
        <div v-reveal class="p-6 md:p-10 lg:p-16 bg-white border border-slate-200 rounded-[3rem] space-y-10 shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
          <!-- Header -->
          <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div class="flex items-center gap-3">
              ...
              <span class="text-[11px] font-black uppercase tracking-[0.3em] text-slate-500">{{ $t('pillarCrmEngageLabel') }}</span>
            </div>
          ...
          <!-- Quote -->
          <p class="text-2xl md:text-3xl text-slate-900 font-serif italic leading-snug max-w-5xl">"{{ $t('pillarCrmQuote') }}"</p>

          <!-- 3 Capabilities -->
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div v-for="(cap, i) in crmCaps" :key="i" class="p-8 bg-slate-50 border border-slate-200 rounded-[2rem] space-y-4 hover:bg-slate-100 transition-all">
              <div class="w-12 h-12 rounded-xl bg-[#7F39E9]/10 flex items-center justify-center text-[#7F39E9]">
                <Icon :icon="cap.icon" class="text-xl" />
              </div>
              <h4 class="text-xl font-black text-slate-900 tracking-tight">{{ $t(cap.titleKey) }}</h4>
              <p class="text-sm text-slate-500 font-light leading-relaxed">{{ $t(cap.descKey) }}</p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8 pt-8 border-t border-slate-100">
            <div v-for="(stat, i) in crmStats" :key="i">
              <div class="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter">{{ stat.val }}</div>
        ...
        <div v-reveal class="p-6 md:p-10 lg:p-16 rounded-[3rem] relative overflow-hidden" style="background: linear-gradient(224.95deg, #a446f4 -1.95%, #4138f3 104.5%); border: 1px solid rgba(255,255,255,0.15)">
        ...
        <div v-reveal class="p-6 md:p-10 lg:p-16 bg-white border border-slate-200 rounded-[3rem] space-y-10 shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div class="flex items-center gap-3">
              ...
              <span class="text-[11px] font-black uppercase tracking-[0.3em] text-slate-500">{{ $t('pillarCrmGrowLabel') }}</span>
            </div>
            <div class="px-3 py-1.5 rounded-full bg-teal-500/10 border border-teal-500/30 text-teal-600 text-[10px] font-black uppercase tracking-[0.2em] shadow-[0_2px_10px_rgba(20,184,166,0.2)]">{{ $t('pillarActiveModule') }}</div>
          </div>

          <!-- Quote -->
          <p class="text-2xl md:text-3xl text-slate-900 font-serif italic leading-snug max-w-5xl">"{{ $t('pillarCrmGrowQuote') }}"</p>

          <!-- 3 Capabilities -->
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div v-for="(cap, i) in crmGrowCaps" :key="i" class="p-8 bg-slate-50 border border-slate-200 rounded-[2rem] space-y-4 hover:bg-slate-100 transition-all">
              <div class="w-12 h-12 rounded-xl bg-[#7F39E9]/10 flex items-center justify-center text-[#7F39E9]">
                <Icon :icon="cap.icon" class="text-xl" />
              </div>
              <h4 class="text-xl font-black text-slate-900 tracking-tight">{{ $t(cap.titleKey) }}</h4>
          ...
          <div class="pt-8 border-t border-slate-100 text-center">
            <div class="text-5xl md:text-7xl font-black text-slate-900 tracking-tighter leading-none">+$12.40</div>
        ...
        <div v-reveal class="p-6 md:p-12 lg:p-20 rounded-[3rem] text-center space-y-8 relative overflow-hidden" style="background: linear-gradient(224.95deg, #a446f4 -1.95%, #4138f3 104.5%); border: 1px solid rgba(255,255,255,0.15)">
          ...
          <h2 class="text-3xl md:text-5xl font-black tracking-tighter leading-tight max-w-4xl mx-auto" style="color: #7dd3fc">{{ $t('pillarCrmCloseLine2') }}</h2>
      ...
      <div v-if="pillarId === 'support'" class="space-y-24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div class="space-y-8">
            ...
            <h1 class="text-4xl sm:text-6xl md:text-8xl font-black text-slate-900 leading-tight md:leading-[0.85] tracking-tighter">Support<br />AI.</h1>
            <h2 class="text-2xl md:text-3xl font-bold text-slate-700 tracking-tight leading-tight max-w-md">{{ $t('pillarSupportHeroSubtitle') }}</h2>
            <p class="text-xl text-slate-500 font-light max-w-xl leading-relaxed">{{ $t('pillarSupportHero') }}</p>
            <PillarBadges />
          </div>
          <div class="relative aspect-square flex items-center justify-center">
            <SupportVisual />
        ...
        <div v-reveal class="p-6 md:p-12 lg:p-20 bg-slate-50 border border-slate-200 rounded-[3rem] text-center space-y-12">
          <div class="space-y-4">
            <div class="text-[12px] font-black text-[#7F39E9] uppercase tracking-[0.5em]">{{ $t('pillarSupportGapLabel') }}</div>
            <h2 class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter leading-none max-w-4xl mx-auto">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
            <div class="p-8 bg-rose-500/5 border border-rose-500/10 rounded-[2rem] space-y-6 text-left">
              ...
              <ul class="space-y-3">
                <li v-for="(item, i) in supportOldKeys" :key="i" class="flex items-center gap-3 text-slate-600 font-medium text-sm">
            ...
            <div class="p-8 bg-[#7F39E9]/5 border border-[#7F39E9]/30 rounded-[2rem] space-y-6 text-left">
              <h3 class="text-xl font-black text-slate-900 tracking-tight">{{ $t('pillarSupportImpulse') }}</h3>
              <ul class="space-y-3">
                <li v-for="(item, i) in supportNewKeys" :key="i" class="flex items-center gap-3 text-slate-900 font-bold text-sm">
          ...
          <div class="pt-8">
            <div class="text-4xl sm:text-6xl md:text-7xl lg:text-9xl font-black text-slate-900 tracking-tighter leading-none">{{ $t('pillarSupportBigStat') }}</div>
        ...
        <div v-reveal class="space-y-12">
          <div class="text-center space-y-6 max-w-3xl mx-auto">
            ...
            <h2 class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter leading-none">{{ $t('pillarSupportOmniTitle') }}</h2>
            <p class="text-xl text-slate-500 font-light leading-relaxed">{{ $t('pillarSupportOmniDesc') }}</p>
          </div>
          ...
          <div class="relative h-[450px] w-full flex items-center justify-center overflow-hidden">
            ...
            <div v-for="(ch, i) in supportChannels" :key="i"
              class="absolute w-14 h-14 rounded-2xl bg-white border border-slate-200 flex items-center justify-center shadow-xl text-[#7F39E9] hover:scale-110 hover:border-[#7F39E9]/50 transition-all cursor-default"
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 max-w-6xl mx-auto">
            <div v-for="(ch, i) in supportChannels" :key="'grid-'+i" class="p-6 bg-white border border-slate-200 rounded-[2rem] space-y-4 text-center hover:border-[#7F39E9]/30 hover:bg-[#7F39E9]/5 transition-all">
              <div class="w-12 h-12 mx-auto rounded-xl bg-[#7F39E9]/10 flex items-center justify-center text-[#7F39E9]">
                <Icon :icon="ch.icon" class="text-xl" />
              </div>
              <h4 class="text-sm font-black text-slate-900 uppercase tracking-wider">{{ $t(ch.titleKey) }}</h4>
              <p class="text-xs text-slate-500 font-light leading-relaxed">{{ $t(ch.descKey) }}</p>
            </div>
          </div>
        </div>

        <!-- SECTION 4: CONTEXT ENGINEERING -->
        <div v-reveal class="p-6 md:p-10 lg:p-16 bg-white border border-slate-200 rounded-[3rem] space-y-12">
          <div class="text-center space-y-4 max-w-3xl mx-auto">
            <div class="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">{{ $t('pillarSupportContextLabel') }}</div>
            <h2 class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter leading-none">{{ $t('pillarSupportContextTitle') }}</h2>
          </div>
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="(ctx, i) in supportContextCards" :key="i" class="p-8 bg-slate-50 border border-slate-200 rounded-[2rem] space-y-4 hover:bg-slate-100 transition-all">
              <div class="w-12 h-12 rounded-xl bg-[#7F39E9]/10 flex items-center justify-center text-[#7F39E9]">
                <Icon :icon="ctx.icon" class="text-xl" />
              </div>
              <h4 class="text-lg font-black text-slate-900 tracking-tight">{{ $t(ctx.titleKey) }}</h4>
              <p class="text-sm text-slate-500 font-light leading-relaxed">{{ $t(ctx.descKey) }}</p>
            </div>
          </div>
        </div>
        ...
        <div v-reveal class="space-y-10">
          <div class="text-center space-y-4">
            ...
            <h2 class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter leading-none">{{ $t('pillarSupportCopilotTitle') }}</h2>
            <p class="text-xl text-slate-500 font-light max-w-3xl mx-auto leading-relaxed">{{ $t('pillarSupportCopilotDesc') }}</p>
          </div>

          <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-slate-100 p-8 lg:p-14 rounded-[3rem] border border-slate-200 relative overflow-hidden">
            <!-- Left: Live Chat -->
            <div class="space-y-6 relative z-10">
              ...
              <div class="flex justify-start animate-fade-up" style="opacity: 0; animation-delay: 1.5s; animation-fill-mode: forwards">
                <div class="max-w-[85%] p-5 bg-[#7F39E9]/10 border border-[#7F39E9]/20 rounded-2xl rounded-tl-none space-y-3">
                  ...
                  <p class="text-slate-900 font-medium text-sm leading-relaxed">{{ $t('pillarSupportCopilotAI') }}</p>
                  <div class="flex gap-2">
                    ...
                    <button class="px-4 py-2 bg-slate-200 rounded-lg text-[10px] font-black uppercase tracking-widest text-slate-900">Edit</button>
                  </div>
                </div>
              </div>
            ...
            <div class="bg-white border border-slate-200 rounded-[2rem] p-8 space-y-8 relative z-10">
              <div class="flex items-center justify-between">
                <h3 class="text-lg font-black text-slate-900 tracking-tight">Network Context</h3>
              ...
              <div class="space-y-4">
                <div v-for="(row, i) in supportCopilotData" :key="i" class="flex items-center justify-between py-2.5 border-b border-slate-100 last:border-0">
                  <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">{{ row.label }}</span>
      ...
      <div v-if="pillarId === 'billing'" class="space-y-24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div class="space-y-8">
            ...
            <h1 class="text-4xl sm:text-6xl md:text-8xl font-black text-slate-900 leading-tight md:leading-[0.85] tracking-tighter">Billing<br />AI.</h1>
            <h2 class="text-2xl md:text-3xl font-bold text-slate-700 tracking-tight leading-tight max-w-md">{{ $t('pillarBillingHeroSubtitle') }}</h2>
            <p class="text-xl text-slate-500 font-light max-w-xl leading-relaxed">{{ $t('pillarBillingHero') }}</p>
            <PillarBadges />
          </div>
          <div class="relative aspect-square flex items-center justify-center">
            <BillingVisual />
        ...
        <div v-reveal class="p-6 md:p-12 lg:p-20 bg-slate-50 border border-slate-200 rounded-[3rem] text-center space-y-12">
          <div class="space-y-6 max-w-4xl mx-auto">
            ...
            <h2 class="text-3xl md:text-5xl font-black text-slate-900 tracking-tighter leading-tight">
              {{ $t('pillarBillingGapTitle') }}
            </h2>
            <p class="text-xl text-slate-500 font-light leading-relaxed max-w-3xl mx-auto">{{ $t('pillarBillingGapDesc') }}</p>
          </div>
          <div class="flex flex-wrap items-center justify-center gap-6">
            <div v-for="(stat, i) in billingStatPills" :key="i" class="px-8 py-4 rounded-full bg-white border border-slate-200 text-slate-900 text-sm font-black uppercase tracking-widest shadow-sm">
              {{ stat }}
            </div>
          </div>
        </div>
        ...
        <div v-reveal class="space-y-12 text-center">
          <div class="space-y-6 max-w-3xl mx-auto">
            <div class="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">{{ $t('pillarBillingCoreLabel') }}</div>
            <h2 class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter leading-none">{{ $t('pillarBillingCoreTitle') }}</h2>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="(cap, i) in billingCaps" :key="i" class="p-8 bg-slate-50 border border-slate-200 rounded-[2.5rem] space-y-5 hover:bg-slate-100 transition-all text-left">
              ...
              <h3 class="text-xl font-black text-slate-900 tracking-tight">{{ $t(cap.titleKey) }}</h3>
              <p class="text-sm text-slate-500 font-light leading-relaxed">{{ $t(cap.descKey) }}</p>
            </div>
          </div>
        </div>
        ...
        <div v-reveal class="space-y-12 text-center">
          <div class="space-y-6 max-w-3xl mx-auto">
            <div class="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">{{ $t('pillarBillingPricingLabel') }}</div>
            <h2 class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter leading-none">{{ $t('pillarBillingPricingTitle') }}</h2>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            <div v-for="(card, i) in billingPricingCards" :key="i" class="p-8 rounded-[2rem] space-y-4 text-left transition-all" :class="card.accent ? 'bg-[#7F39E9] border border-[#7F39E9]' : 'bg-slate-50 border border-slate-200 hover:bg-slate-100'">
              <div class="w-12 h-12 rounded-xl flex items-center justify-center" :class="card.accent ? 'bg-white/20 text-white' : 'bg-[#7F39E9]/10 text-[#7F39E9]'">
                <Icon :icon="card.icon" class="text-xl" />
              </div>
              <h4 class="text-xl font-black tracking-tight" :class="card.accent ? 'text-white' : 'text-slate-900'">{{ $t(card.titleKey) }}</h4>
              <p class="text-sm font-light leading-relaxed" :class="card.accent ? 'text-white/70' : 'text-slate-500'">{{ $t(card.descKey) }}</p>
            </div>
          </div>
        </div>
        ...
        <div v-reveal class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div class="p-10 bg-white border border-slate-200 rounded-[3rem] space-y-8">
            <div class="space-y-4">
              <div class="flex items-center gap-3">
                ...
                <span class="text-[11px] font-black uppercase tracking-[0.3em] text-slate-500">{{ $t('pillarBillingContractsLabel') }}</span>
              </div>
              <h3 class="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter">{{ $t('pillarBillingContractsTitle') }}</h3>
              <p class="text-sm text-slate-500 font-light leading-relaxed">{{ $t('pillarBillingContractsDesc') }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
              <div v-for="(step, i) in billingContractSteps" :key="i" class="flex items-center gap-3">
                <div class="px-5 py-2.5 rounded-full text-sm font-black uppercase tracking-wider" :class="step.active ? 'bg-[#7F39E9] text-white' : 'bg-slate-100 border border-slate-200 text-slate-700'">
                  {{ $t(step.labelKey) }}
                </div>
                <Icon v-if="i < billingContractSteps.length - 1" icon="heroicons-outline:chevron-right" class="text-slate-300 text-base" />
              </div>
            </div>
          </div>
          <!-- Right: Impulse Pay -->
          <div class="p-10 bg-white border border-slate-200 rounded-[3rem] space-y-8">
            <div class="space-y-4">
              <div class="flex items-center gap-3">
                ...
                <span class="text-[11px] font-black uppercase tracking-[0.3em] text-slate-500">{{ $t('pillarBillingPayLabel') }}</span>
              </div>
              <h3 class="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter">{{ $t('pillarBillingPayTitle') }}</h3>
              <p class="text-sm text-slate-500 font-light leading-relaxed">{{ $t('pillarBillingPayDesc') }}</p>
            </div>
            <div class="flex flex-wrap gap-3">
              <span v-for="(feat, i) in billingPayFeats" :key="i" class="px-4 py-2 rounded-full bg-slate-50 border border-slate-200 text-xs font-bold text-slate-700">
                {{ $t(feat) }}
              </span>
            </div>
          </div>
      ...
      <div v-if="pillarId === 'advisory'" class="space-y-24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div class="space-y-8">
            ...
            <h1 class="text-4xl sm:text-6xl md:text-8xl font-black text-slate-900 leading-tight md:leading-[0.85] tracking-tighter">Impulse<br />Advisory.</h1>
            <h2 class="text-2xl md:text-3xl font-bold text-slate-700 tracking-tight leading-tight max-w-md">{{ $t('pillarAdvisoryHeroSubtitle') }}</h2>
            <p class="text-xl text-slate-500 font-light max-w-xl leading-relaxed">{{ $t('pillarAdvisoryHero') }}</p>
            <PillarBadges />
          </div>
          <div class="relative aspect-square flex items-center justify-center">
            <AdvisoryVisual />
        ...
        <div v-reveal class="p-6 md:p-10 lg:p-16 bg-white border border-slate-200 rounded-[3rem] space-y-12">
          <div class="text-center space-y-6 max-w-3xl mx-auto">
            <div class="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">{{ $t('pillarAdvisoryFlywheelLabel') }}</div>
            <h2 class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter leading-none">{{ $t('pillarAdvisoryFlywheelTitle') }}</h2>
            <p class="text-xl text-slate-500 font-light leading-relaxed">{{ $t('pillarAdvisoryFlywheelDesc') }}</p>
          </div>
          <div class="relative grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
            <!-- Connecting line (desktop only) -->
            <div class="hidden md:block absolute top-8 left-[12.5%] right-[12.5%] h-px bg-slate-200 z-0" />
            <div v-for="(step, i) in advisorySteps" :key="i" class="relative z-10 text-center space-y-4">
              ...
              <h4 class="text-lg font-black text-slate-900 tracking-tight">{{ $t(step.titleKey) }}</h4>
              <p class="text-sm text-slate-500 font-light leading-relaxed">{{ $t(step.descKey) }}</p>
            </div>
          </div>
        </div>
        ...
        <div v-reveal class="p-6 md:p-12 lg:p-20 rounded-[3rem] space-y-12" style="background: linear-gradient(224.95deg, #a446f4 -1.95%, #4138f3 104.5%); border: 1px solid rgba(255,255,255,0.15)">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div
              ...
              :class="svc.highlighted ? 'bg-white border-2 border-amber-400/50' : 'bg-white/10 border border-white/10'"
            >
              ...
              <h4 class="text-xl font-black tracking-tight" :class="svc.highlighted ? 'text-slate-900' : 'text-white'">{{ $t(svc.titleKey) }}</h4>
              <p class="text-sm font-light leading-relaxed" :class="svc.highlighted ? 'text-slate-500' : 'text-white/60'">{{ $t(svc.descKey) }}</p>
              <div class="flex flex-wrap items-center gap-3 pt-2">
                <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest" :class="svc.highlighted ? 'bg-slate-100 text-slate-700' : 'bg-white/10 text-white/80'">{{ svc.price }}</span>
                <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest" :class="svc.highlighted ? 'bg-slate-100 text-slate-700' : 'bg-white/10 text-white/80'">{{ svc.timeline }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            <div
              ...
              :key="i + 3"
              class="p-8 rounded-[2rem] bg-white/10 border border-white/10 space-y-5 transition-all"
            >
...
export default {
  ...
  data() {
    return {
      ...
      supportCopilotData: [
        { label: 'Device', value: 'ZTE F670L', color: 'text-slate-900' },
        ...
        { label: 'Tenure', value: '14 months', color: 'text-slate-900' },
        { label: 'CLV', value: '$756/yr', color: 'text-slate-900' }
      ],
      supportCloseStats: [
